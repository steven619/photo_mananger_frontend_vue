<template>
  ...
  >
    ...
    >
      ...
      >
        {{ $t('route.' + tag.meta.title).indexOf('.') > -1 ?
          $t('route.' + tag.meta.title).split('.').slice(-1)[0]:$t('route.' + tag.meta.title) }}
        <span
          v-if="!tag.meta.affix"
          class="el-icon-close"
...
export default class extends Vue {
  ...
  private addTags() {
    ...
    if (name) {
      TagsViewModule.addView(this.$route as any)
    }
    return false
  }
      ...
      for (const tag of tags) {
        if ((tag.to as ITagView).path === this.$route.path) {
          ...
          if ((tag.to as ITagView).fullPath !== this.$route.fullPath) {
            TagsViewModule.updateVisitedView(this.$route as any)
          }
          break
        }
      }
