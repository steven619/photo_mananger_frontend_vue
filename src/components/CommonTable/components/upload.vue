...
export default {
  ...
  data() {
    return {
      ...
      defaultConfig: {
        ...
        resField: 'data', // 获取返回结果的字段,
        fileNameField: 'name',
        fileUrlField: 'url',
        fileField: 'file',
        stringValue: false, // 输入输出为string的开关
        headers: {}
      },
      uploadInfo: {
        numUploaded: 0,
  ...
  computed: {
    ...
    fileList() {
      const vm = this
      const modelVal = this.modelValTmp
      let numUploaded = 0
  ...
  created() {
    if (!this.modelVal) {
      this.modelValTmp = []
    } else if (this.modelVal && typeof this.modelVal === 'string') {
      this.modelValTmp = [{ name: '', url: imgBaseUrl + this.modelVal }]
    } else {
      this.modelValTmp = this.modelVal.map(e => { return { name: e.split('/').slice(-1)[0], url: imgBaseUrl + e } })
    }
  },
  ...
  methods: {
    ...
    uploadCallback({ state, fileList }) {
      ...
      if (vm.uploadInfo.count === total) { // 完成上传
        ...
        this.modelValTmp = fileList
        if (this.mergeConfig.stringValue && fileList.length > 0) {
          vm.modelVal = fileList[0].url
        } else {
          vm.modelVal = fileList.map(e => { return e.url })
        }
